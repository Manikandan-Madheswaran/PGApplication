...
@RestController
public class BankHostedController
{
	@Autowired
	...
	@RequestMapping(value = "/BankHosted")
	...
	@RequestMapping(value = "/BankHostedTransaction")
	public ModelAndView performHostedTransaction(HttpServletRequest request,HttpServletResponse response,TransactionRequest tranData)
	{
		...
		try
		{
			...
			tranData.setResponseURL(ConfigFileUtil.getProperty("CONTEXT_PATH")+"BankHostedResponse");
			tranData.setErrorURL(ConfigFileUtil.getProperty("CONTEXT_PATH")+"BankHostedResponse");
	...
	@RequestMapping(value = "/BankHostedResponse")
