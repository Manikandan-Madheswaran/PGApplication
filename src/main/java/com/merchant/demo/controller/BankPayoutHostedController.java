...
@RestController
public class BankPayoutHostedController
{
	@Autowired
	...
	@RequestMapping(value = "/BankHostedPayout")
	public ModelAndView HostedRequest(HttpServletRequest request,HttpServletResponse response)
	{
		...
		try
		{
			...
			mav.setViewName("BankHostedPayout");
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	@RequestMapping(value = "/BankHostedPayoutTransaction")
	public ModelAndView performHostedTransaction(HttpServletRequest request,HttpServletResponse response,TransactionRequest tranData)
	{
		...
		try
		{
			...
			tranData.setResponseURL(ConfigFileUtil.getProperty("CONTEXT_PATH")+"BankHostedPayoutResponse");
			tranData.setErrorURL(ConfigFileUtil.getProperty("CONTEXT_PATH")+"BankHostedPayoutResponse");
	...
	@RequestMapping(value = "/BankHostedPayoutResponse")
